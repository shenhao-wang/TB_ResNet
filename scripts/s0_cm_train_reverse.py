...
data['training'] = {}
data['testing'] = {}
data['training'] = [np.concatenate(
    [cm_data['training'][x0_vars].values, cm_data['training'][x1_vars].values, cm_data['training'][x2_vars].values,
     cm_data['training'][x3_vars].values, cm_data['training'][x4_vars].values, cm_data['training'][z_vars].values],
    axis=1), cm_data['training'][y_vars].values[:, 0]]
data['testing'] = [np.concatenate(
    [cm_data['testing'][x0_vars].values, cm_data['testing'][x1_vars].values, cm_data['testing'][x2_vars].values,
     cm_data['testing'][x3_vars].values, cm_data['testing'][x4_vars].values, cm_data['testing'][z_vars].values],
    axis=1), cm_data['testing'][y_vars].values[:, 0]]
data_grid_dnn = {}
data_grid_dnn['training'] = [np.concatenate(
    [data_grid['training'][0], data_grid['training'][1], data_grid['training'][2], data_grid['training'][3],
     data_grid['training'][4], data_grid['training'][5]], axis=1), data_grid['training'][-1]]
...
for penalty_const in penalty_const_list:
    ...
    # 1. estimate dnn
    restore = False
    cm_dnn_params_dic, cm_dnn_hyper_params_dic, cm_dnn_output_dic = util.est_dnn(data, penalty_const, MODEL_NAME,
                                                                                 restore, n_epoches=n_epoches, K=K)

    # 2. return grid info
    restore = True
    _, _, cm_dnn_grid_output_dic = util.est_dnn(data_grid_dnn, penalty_const, MODEL_NAME, restore, n_epoches=n_epoches,
                                                K=K)
    ...
    for epsilon in epsilon_list:
        ...
        gradient_training = cm_dnn_output_dic['gradient_cost_x_training']
        gradient_testing = cm_dnn_output_dic['gradient_cost_x_testing']
        data_adv_fgsm = util_helpers.fgsm_adv_examples(data, data_len_list, epsilon, gradient_training,
                                                       gradient_testing)
        restore = True
        _, _, cm_dnn_adv_fgsm_output = util.est_dnn(data_adv_fgsm, penalty_const, MODEL_NAME, restore, n_epoches, K=K)
    ...
    for epsilon in epsilon_list:
        ...
        _, _, cm_dnn_adv_tgsm_output = util.est_dnn(data_adv_tgsm, penalty_const, MODEL_NAME, restore, n_epoches, K=K)
...
restore = False
cm_est_params_dic,cm_est_hyper_params_dic,cm_est_output_dic = util.est_cm(data, MODEL_NAME, restore, n_epoches)
...
for epsilon in epsilon_list:
    print('fgsm ', epsilon)
    data_len_list = [len(x0_vars), len(x1_vars), len(x2_vars), len(x3_vars), len(x4_vars), len(z_vars)]
    gradient_training = cm_est_output_dic['gradient_cost_x_training']
    gradient_testing = cm_est_output_dic['gradient_cost_x_testing']
    ...
    cm_est_adv_fgsm_output_dic[epsilon] = {'accuracy_training': cm_est_adv_fgsm_output['accuracy_training'], 'accuracy_testing': cm_est_adv_fgsm_output['accuracy_testing']}
...
for epsilon in epsilon_list:
    ...
    cm_est_adv_tgsm_output_dic[epsilon] = {'accuracy_training':cm_est_adv_tgsm_output['accuracy_training'], 'accuracy_testing':cm_est_adv_tgsm_output['accuracy_testing']}
...
for penalty_const in penalty_const_list:
    ### 
    MODEL_NAME = 'cm_resnet_reverse'+str(penalty_const)

    # 1. estimate cm_resent
    restore = False
    dnn_param_dic = cm_dnn_params_dic # this is estimated from cm_est
    cm_resnet_params_dic,cm_resnet_hyper_params_dic,cm_resnet_output_dic = util.est_cm_resnet_reverse(data, dnn_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)

    # 2. restore 
    restore = True
    _,_,cm_resnet_grid_output_dic = util.est_cm_resnet_reverse(data_grid, dnn_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
    ...
    for epsilon in epsilon_list:
        print('fgsm ', epsilon)
        data_len_list = [len(x0_vars), len(x1_vars), len(x2_vars), len(x3_vars), len(x4_vars), len(z_vars)]
        gradient_training = cm_resnet_output_dic['gradient_cost_x_training']
        gradient_testing = cm_resnet_output_dic['gradient_cost_x_testing']
        data_adv_fgsm = util_helpers.fgsm_adv_examples(data, data_len_list, epsilon, gradient_training, gradient_testing)
        restore = True
        _,_,cm_resnet_adv_fgsm_output=util.est_cm_resnet_reverse(data_adv_fgsm, dnn_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
        cm_resnet_adv_fgsm_output_dic[epsilon] = {'accuracy_training': cm_resnet_adv_fgsm_output['accuracy_training'], 'accuracy_testing': cm_resnet_adv_fgsm_output['accuracy_testing']}
    ...
    for epsilon in epsilon_list:
        print('tgsm ', epsilon)    
        #MODEL_NAME = 'cm_est' # use it to generate the target gradients
        gradient_target_training,gradient_target_testing = util_helpers.tgsm_target_gradients(data, n_class, MODEL_NAME, fun = util.est_cm_resnet_reverse, penalty_const = penalty_const, param_dic = dnn_param_dic)
        data_len_list = [len(x0_vars), len(x1_vars), len(x2_vars), len(x3_vars), len(x4_vars), len(z_vars)]
        restore = True
        data_adv_tgsm =  util_helpers.tgsm_adv_examples(data, data_len_list, epsilon, gradient_target_training, gradient_target_testing)
        _,_,cm_resnet_adv_tgsm_output=util.est_cm_resnet_reverse(data_adv_tgsm, dnn_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
        cm_resnet_adv_tgsm_output_dic[epsilon] = {'accuracy_training':cm_resnet_adv_tgsm_output['accuracy_training'], 'accuracy_testing':cm_resnet_adv_tgsm_output['accuracy_testing']}
...
#
with open('cm_info_reverse.pickle', 'wb') as f:
