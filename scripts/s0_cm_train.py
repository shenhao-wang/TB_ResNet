...
for penalty_const in penalty_const_list:
    ...
    current_best_acc = 0
    for replica in range(num_replications):
        _cm_dnn_params_dic,_cm_dnn_hyper_params_dic,_cm_dnn_output_dic = util.est_dnn(data, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K, current_best_acc = current_best_acc)
    ...
    # 2. return grid info
    restore = True
    _,_,cm_dnn_grid_output_dic = util.est_dnn(data_grid_dnn, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
    ...
    for epsilon in epsilon_list:
        ...
        cm_dnn_adv_fgsm_output_dic[epsilon] = {'accuracy_training': cm_dnn_adv_fgsm_output['accuracy_training'], 'accuracy_testing': cm_dnn_adv_fgsm_output['accuracy_testing']}
    ...
    for epsilon in epsilon_list:
        ...
        cm_dnn_adv_tgsm_output_dic[epsilon] = {'accuracy_training':cm_dnn_adv_tgsm_output['accuracy_training'], 'accuracy_testing':cm_dnn_adv_tgsm_output['accuracy_testing']}
    #
    cm_info[MODEL_NAME] = {}
...
for penalty_const in penalty_const_list:
    ...
    for replica in range(num_replications):
        _cm_resnet_params_dic,_cm_resnet_hyper_params_dic,_cm_resnet_output_dic = \
            util.est_cm_resnet(data, cm_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K,
    ...
    # 2. restore 
    restore = True
    _,_,cm_resnet_grid_output_dic = util.est_cm_resnet(data_grid, cm_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
    ...
    for epsilon in epsilon_list:
        ...
        data_adv_fgsm = util_helpers.fgsm_adv_examples(data, data_len_list, epsilon, gradient_training, gradient_testing)
        restore = True
        _,_,cm_resnet_adv_fgsm_output=util.est_cm_resnet(data_adv_fgsm, cm_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
    ...
    for epsilon in epsilon_list:
        print('tgsm ', epsilon)    
        #MODEL_NAME = 'cm_est' # use it to generate the target gradients
        gradient_target_training,gradient_target_testing = util_helpers.tgsm_target_gradients(data, n_class, MODEL_NAME, fun = util.est_cm_resnet, penalty_const = penalty_const, param_dic = cm_param_dic)
        data_len_list = [len(x0_vars), len(x1_vars), len(x2_vars), len(x3_vars), len(x4_vars), len(z_vars)]
        restore = True
        data_adv_tgsm =  util_helpers.tgsm_adv_examples(data, data_len_list, epsilon, gradient_target_training, gradient_target_testing)
        _,_,cm_resnet_adv_tgsm_output=util.est_cm_resnet(data_adv_tgsm, cm_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
...
#
with open('cm_info.pickle', 'wb') as f:
