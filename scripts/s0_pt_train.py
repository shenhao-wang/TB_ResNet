...
for epsilon in epsilon_list:
    ...
    pt_est_adv_fgsm_output_dic[epsilon] = {'accuracy_training': pt_est_adv_fgsm_output['accuracy_training'], 'accuracy_testing': pt_est_adv_fgsm_output['accuracy_testing']}
...
for epsilon in epsilon_list:
    ...
    pt_est_adv_tgsm_output_dic[epsilon] = {'accuracy_training':pt_est_adv_tgsm_output['accuracy_training'], 'accuracy_testing':pt_est_adv_tgsm_output['accuracy_testing']}
...
#    penalty_const = 0.01
    ...
    # 1. estimate dnn
    MODEL_NAME = 'pt_dnn_'+str(penalty_const)
    restore = False
    ...
    current_best_acc = 0
    for replica in range(num_replications):
        _pt_dnn_params_dic,_pt_dnn_hyper_params_dic,_pt_dnn_output_dic = util.est_dnn(data, penalty_const, MODEL_NAME, restore, n_epoches,
                                                                                   current_best_acc=current_best_acc)
    ...
    # 2. return grid info
    restore = True
    _,_,pt_dnn_grid_output_dic = util.est_dnn(data_grid_dnn, penalty_const, MODEL_NAME, restore, n_epoches)
    #
    ...
    for epsilon in epsilon_list:
        ...
        data_adv_fgsm = util_helpers.fgsm_adv_examples(data, data_len_list, epsilon, gradient_training, gradient_testing)
        restore = True
        _,_,pt_dnn_adv_fgsm_output=util.est_dnn(data_adv_fgsm, penalty_const, MODEL_NAME, restore, n_epoches, K = K)
        pt_dnn_adv_fgsm_output_dic[epsilon] = {'accuracy_training': pt_dnn_adv_fgsm_output['accuracy_training'], 'accuracy_testing': pt_dnn_adv_fgsm_output['accuracy_testing']}
    ...
    for epsilon in epsilon_list:
        print('tgsm ', epsilon)
        #MODEL_NAME = 'pt_est' # use it to generate the target gradients
        gradient_target_training,gradient_target_testing = util_helpers.tgsm_target_gradients(data, n_class, MODEL_NAME, fun = util.est_dnn, penalty_const = penalty_const)
        data_len_list = [len(x0_var_names)+len(x1_var_names)+len(p0_var_names)+len(p1_var_names)+len(z_var_names)]
        restore = True
        data_adv_tgsm =  util_helpers.tgsm_adv_examples(data, data_len_list, epsilon, gradient_target_training, gradient_target_testing)
        _,_,pt_dnn_adv_tgsm_output=util.est_dnn(data_adv_tgsm, penalty_const, MODEL_NAME, restore, n_epoches, K = K)
        pt_dnn_adv_tgsm_output_dic[epsilon] = {'accuracy_training':pt_dnn_adv_tgsm_output['accuracy_training'], 'accuracy_testing':pt_dnn_adv_tgsm_output['accuracy_testing']}
...
#    penalty_const = 0.01
    ...
    current_best_acc = 0
    for replica in range(num_replications):
        _pt_resnet_params_dic, _pt_resnet_hyper_params_dic, _pt_resnet_output_dic = util.est_pt_resnet(data, pt_param_dic,
                                                                                                    penalty_const,
                                                                                                    MODEL_NAME, restore,
                                                                                                    n_epoches,
                                                                                                    current_best_acc=current_best_acc)
    ...
    # 2. restore
    restore = True
    _,_,pt_resnet_grid_output_dic = util.est_pt_resnet(data_grid, pt_param_dic, penalty_const, MODEL_NAME, restore, n_epoches)
    ...
    for epsilon in epsilon_list:
        ...
        data_adv_fgsm = util_helpers.fgsm_adv_examples(data, data_len_list, epsilon, gradient_training, gradient_testing)
        restore = True
        _,_,pt_resnet_adv_fgsm_output=util.est_pt_resnet(data_adv_fgsm, pt_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
        pt_resnet_adv_fgsm_output_dic[epsilon] = {'accuracy_training': pt_resnet_adv_fgsm_output['accuracy_training'], 'accuracy_testing': pt_resnet_adv_fgsm_output['accuracy_testing']}
    ...
    for epsilon in epsilon_list:
        print('tgsm ', epsilon)    
        #MODEL_NAME = 'pt_est' # use it to generate the target gradients
        gradient_target_training,gradient_target_testing = util_helpers.tgsm_target_gradients(data, n_class, MODEL_NAME, fun = util.est_pt_resnet, penalty_const = penalty_const, param_dic = pt_param_dic)
        data_len_list = [len(x0_var_names), len(x1_var_names), len(p0_var_names), len(p1_var_names), len(z_var_names)]
        restore = True
        data_adv_tgsm =  util_helpers.tgsm_adv_examples(data, data_len_list, epsilon, gradient_target_training, gradient_target_testing)
        _,_,pt_resnet_adv_tgsm_output=util.est_pt_resnet(data_adv_tgsm, pt_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
        pt_resnet_adv_tgsm_output_dic[epsilon] = {'accuracy_training':pt_resnet_adv_tgsm_output['accuracy_training'], 'accuracy_testing':pt_resnet_adv_tgsm_output['accuracy_testing']}
...
#
with open('pt_info.pickle', 'wb') as f:
