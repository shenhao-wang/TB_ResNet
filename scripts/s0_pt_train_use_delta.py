...
def model(PT, DNN, RESNET, Load_old_results):
    ...
    if PT:
        ...
        ### attacks
        # 3. fgsm
        pt_est_adv_fgsm_output_dic = {}
        for epsilon in epsilon_list:
            print('fgsm ', epsilon)
            data_len_list = [len(x0_var_names), len(x1_var_names), len(p0_var_names), len(p1_var_names), len(z_var_names)]
            gradient_training = pt_est_output_dic[best_index_pt]['gradient_cost_x_training']
            gradient_testing = pt_est_output_dic[best_index_pt]['gradient_cost_x_testing']
            data_adv_fgsm = util_helpers.fgsm_adv_examples(data, data_len_list, epsilon, gradient_training, gradient_testing)
            restore = True
            _,_,pt_est_adv_fgsm_output=util.est_pt(data_adv_fgsm, MODEL_NAME, restore, n_epoches)
            pt_est_adv_fgsm_output_dic[epsilon] = {'accuracy_training': pt_est_adv_fgsm_output['accuracy_training'],
                                                   'accuracy_testing': pt_est_adv_fgsm_output['accuracy_testing'],
                                                   'log_loss_training': pt_est_adv_fgsm_output['log_loss_training'],
                                                   'log_loss_testing': pt_est_adv_fgsm_output['log_loss_testing']
                                                   }

        # 4. tgsm
        pt_est_adv_tgsm_output_dic = {}
        for epsilon in epsilon_list:
            print('tgsm ', epsilon)
            #MODEL_NAME = 'pt_est' # use it to generate the target gradients
            gradient_target_training,gradient_target_testing = util_helpers.tgsm_target_gradients(data, n_class, MODEL_NAME, fun = util.est_pt)
            data_len_list = [len(x0_var_names), len(x1_var_names), len(p0_var_names), len(p1_var_names), len(z_var_names)]
            restore = True
            data_adv_tgsm =  util_helpers.tgsm_adv_examples(data, data_len_list, epsilon, gradient_target_training, gradient_target_testing)
            _,_,pt_est_adv_tgsm_output=util.est_pt(data_adv_tgsm, MODEL_NAME, restore, n_epoches)
            pt_est_adv_tgsm_output_dic[epsilon] = {'accuracy_training':pt_est_adv_tgsm_output['accuracy_training'],
                                                   'accuracy_testing':pt_est_adv_tgsm_output['accuracy_testing'],
                                                   'log_loss_training': pt_est_adv_tgsm_output['log_loss_training'],
                                                   'log_loss_testing': pt_est_adv_tgsm_output['log_loss_testing']
                                                   }

            # 5. GN
        pt_est_adv_GN_output_dic = {}
        for epsilon in epsilon_list:
            print('GN ', epsilon)
            # MODEL_NAME = 'pt_est' # use it to generate the target gradients
            data_len_list = [len(x0_var_names), len(x1_var_names), len(p0_var_names), len(p1_var_names), len(z_var_names)]
            ...
            _, _, pt_est_adv_GN_output = util.est_pt(data_adv_GN, MODEL_NAME, restore, n_epoches)
            pt_est_adv_GN_output_dic[epsilon] = {'accuracy_training': pt_est_adv_GN_output['accuracy_training'],
                                                 'accuracy_testing': pt_est_adv_GN_output['accuracy_testing'],
                                                 'log_loss_training': pt_est_adv_GN_output['log_loss_training'],
                                                 'log_loss_testing': pt_est_adv_GN_output['log_loss_testing']
                                                 }
        ...
        pt_info[MODEL_NAME]['fgsm_output']=pt_est_adv_fgsm_output_dic
        pt_info[MODEL_NAME]['tgsm_output']=pt_est_adv_tgsm_output_dic
        pt_info[MODEL_NAME]['GN_output']=pt_est_adv_GN_output_dic
    ...
    if DNN:
        ...
        # 3. fgsm methods
        pt_dnn_adv_fgsm_output_dic = {}
        for epsilon in epsilon_list:
            print('fgsm ', epsilon)
            data_len_list = [len(x0_var_names)+len(x1_var_names)+len(p0_var_names)+len(p1_var_names)+len(z_var_names)]
            gradient_training = pt_dnn_output_dic[best_index]['gradient_cost_x_training']
            gradient_testing = pt_dnn_output_dic[best_index]['gradient_cost_x_testing']
            data_adv_fgsm = util_helpers.fgsm_adv_examples(data, data_len_list, epsilon, gradient_training, gradient_testing)
            restore = True
            _,_,pt_dnn_adv_fgsm_output=util.est_dnn(data_adv_fgsm, l2_reg, MODEL_NAME, restore, n_epoches, K = K)
            pt_dnn_adv_fgsm_output_dic[epsilon] = {'accuracy_training': pt_dnn_adv_fgsm_output['accuracy_training'],
                                                   'accuracy_testing': pt_dnn_adv_fgsm_output['accuracy_testing'],
                                                   'log_loss_training': pt_dnn_adv_fgsm_output['log_loss_training'],
                                                   'log_loss_testing': pt_dnn_adv_fgsm_output['log_loss_testing']
                                                   }

        # 4. tgsm methods
        pt_dnn_adv_tgsm_output_dic = {}
        for epsilon in epsilon_list:
            print('tgsm ', epsilon)
            #MODEL_NAME = 'pt_est' # use it to generate the target gradients
            ...
            restore = True
            data_adv_tgsm =  util_helpers.tgsm_adv_examples(data, data_len_list, epsilon, gradient_target_training, gradient_target_testing)
            _,_,pt_dnn_adv_tgsm_output=util.est_dnn(data_adv_tgsm, l2_reg, MODEL_NAME, restore, n_epoches, K = K)
            pt_dnn_adv_tgsm_output_dic[epsilon] = {'accuracy_training':pt_dnn_adv_tgsm_output['accuracy_training'],
                                                   'accuracy_testing':pt_dnn_adv_tgsm_output['accuracy_testing'],
                                                   'log_loss_training': pt_dnn_adv_tgsm_output['log_loss_training'],
                                                   'log_loss_testing': pt_dnn_adv_tgsm_output['log_loss_testing']
                                                   }

        pt_dnn_adv_GN_output_dic = {}
        for epsilon in epsilon_list:
            print('GN ', epsilon)
            #MODEL_NAME = 'pt_est' # use it to generate the target gradients
            data_len_list = [len(x0_var_names)+len(x1_var_names)+len(p0_var_names)+len(p1_var_names)+len(z_var_names)]
            restore = True
            data_adv_GN =  util_helpers.GN_adv_examples(data, data_len_list, epsilon)
            _,_,pt_dnn_adv_GN_output=util.est_dnn(data_adv_GN, l2_reg, MODEL_NAME, restore, n_epoches, K = K)
            pt_dnn_adv_GN_output_dic[epsilon] = {'accuracy_training':pt_dnn_adv_GN_output['accuracy_training'],
                                                 'accuracy_testing':pt_dnn_adv_GN_output['accuracy_testing'],
                                                 'log_loss_training': pt_dnn_adv_GN_output['log_loss_training'],
                                                 'log_loss_testing': pt_dnn_adv_GN_output['log_loss_testing']
                                                 }
        ...
        pt_info[MODEL_NAME]['fgsm_output']=pt_dnn_adv_fgsm_output_dic
        pt_info[MODEL_NAME]['tgsm_output']=pt_dnn_adv_tgsm_output_dic
        pt_info[MODEL_NAME]['GN_output'] = pt_dnn_adv_GN_output_dic
    ...
    if RESNET:
        ...
        #    penalty_const = 0.01
            ...
            current_best_acc = 0
            for replica in range(num_replications):
                _pt_resnet_params_dic, _pt_resnet_hyper_params_dic, _pt_resnet_output_dic = util.est_pt_resnet_use_delta(data, pt_param_dic,
                                                                                                            penalty_const,
                                                                                                            MODEL_NAME, restore,
                                                                                                            n_epoches,
                                                                                                            current_best_acc=current_best_acc)
            ...
            # 2. restore
            restore = True
            _,_,pt_resnet_grid_output_dic = util.est_pt_resnet_use_delta(data_grid, pt_param_dic, penalty_const, MODEL_NAME, restore, n_epoches)
            ...
            for epsilon in epsilon_list:
                ...
                data_adv_fgsm = util_helpers.fgsm_adv_examples(data, data_len_list, epsilon, gradient_training, gradient_testing)
                restore = True
                _,_,pt_resnet_adv_fgsm_output=util.est_pt_resnet_use_delta(data_adv_fgsm, pt_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
            ...
            for epsilon in epsilon_list:
                ...
                gradient_target_training,gradient_target_testing = util_helpers.tgsm_target_gradients(data, n_class, MODEL_NAME,
                                                                                                      fun = util.est_pt_resnet_use_delta,
                                                                                                      penalty_const = penalty_const, param_dic = pt_param_dic)
                data_len_list = [len(x0_var_names), len(x1_var_names), len(p0_var_names), len(p1_var_names), len(z_var_names)]
                restore = True
                data_adv_tgsm =  util_helpers.tgsm_adv_examples(data, data_len_list, epsilon, gradient_target_training, gradient_target_testing)
                _,_,pt_resnet_adv_tgsm_output=util.est_pt_resnet_use_delta(data_adv_tgsm, pt_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
            ...
            for epsilon in epsilon_list:
                ...
                restore = True
                data_adv_GN =  util_helpers.GN_adv_examples(data, data_len_list, epsilon)
                _,_,pt_resnet_adv_GN_output=util.est_pt_resnet_use_delta(data_adv_GN, pt_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
    ...
    #
    with open('pt_info_use_delta.pickle', 'wb') as f:
