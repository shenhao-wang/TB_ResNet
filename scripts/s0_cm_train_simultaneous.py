...
for penalty_const in penalty_const_list:
    ...
    for replica in range(num_replications):
        _cm_resnet_params_dic, _cm_resnet_hyper_params_dic, _cm_resnet_output_dic = util.est_cm_resnet_simultaneous(data,
                                                                                                                 penalty_const,
                                                                                                                 MODEL_NAME,
                                                                                                                 restore,
                                                                                                                 n_epoches=n_epoches,
    ...
    # 2. restore
    restore = True
    _, _, cm_resnet_grid_output_dic = util.est_cm_resnet_simultaneous(data_grid, penalty_const, MODEL_NAME, restore,
    ...
    for epsilon in epsilon_list:
        ...
        data_adv_fgsm = util_helpers.fgsm_adv_examples(data, data_len_list, epsilon, gradient_training,
                                                       gradient_testing)
        restore = True
        _, _, cm_resnet_adv_fgsm_output = util.est_cm_resnet_simultaneous(data_adv_fgsm, penalty_const, MODEL_NAME,
    ...
    for epsilon in epsilon_list:
        ...
        gradient_target_training, gradient_target_testing = util_helpers.tgsm_target_gradients(data, n_class,
                                                                                               MODEL_NAME,
                                                                                               fun=util.est_cm_resnet_simultaneous,
        ...
        _, _, cm_resnet_adv_tgsm_output = util.est_cm_resnet_simultaneous(data_adv_tgsm, penalty_const, MODEL_NAME,
                                                             restore, n_epoches=n_epoches, K=K)
...
#
with open('cm_info_simul.pickle', 'wb') as f:
