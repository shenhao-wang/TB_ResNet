...
for epsilon in epsilon_list:
    ...
    cm_est_adv_GN_output_dic[epsilon] = {'accuracy_training': cm_est_adv_GN_output['accuracy_training'], 'accuracy_testing': cm_est_adv_GN_output['accuracy_testing']}
...
for penalty_const in penalty_const_list:
    ...
    for epsilon in epsilon_list:
        ...
        restore = True
        data_adv_GN =  util_helpers.GN_adv_examples(data, data_len_list, epsilon)
        _,_,cm_dnn_adv_GN_output=util.est_dnn(data_adv_GN, penalty_const, MODEL_NAME, restore, n_epoches, K = K)
        cm_dnn_adv_GN_output_dic[epsilon] = {'accuracy_training':cm_dnn_adv_GN_output['accuracy_training'], 'accuracy_testing':cm_dnn_adv_GN_output['accuracy_testing']}
...
for penalty_const in penalty_const_list:
    ...
    for epsilon in epsilon_list:
        ...
        restore = True
        data_adv_GN =  util_helpers.GN_adv_examples(data, data_len_list, epsilon)
        _,_,cm_resnet_adv_GN_output=util.est_cm_resnet(data_adv_GN, cm_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
        cm_resnet_adv_GN_output_dic[epsilon] = {'accuracy_training':cm_resnet_adv_GN_output['accuracy_training'], 'accuracy_testing':cm_resnet_adv_GN_output['accuracy_testing']}
...
#
with open('cm_info_GN.pickle', 'wb') as f:
