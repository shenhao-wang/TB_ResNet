...
cm_info[MODEL_NAME]['fgsm_output']= {}
cm_info[MODEL_NAME]['tgsm_output']= {}
cm_info[MODEL_NAME]['GN_output']= {}
...
data_grid_dnn = {}
data_grid_dnn['training']= [np.concatenate([data_grid['training'][0], data_grid['training'][1], data_grid['training'][2],
                                            data_grid['training'][3], data_grid['training'][4], data_grid['training'][5]], axis = 1), data_grid['training'][-1]]
...
current_best_acc = 0
for replica in range(num_replications):
    _cm_dnn_params_dic,_cm_dnn_hyper_params_dic,_cm_dnn_output_dic = util.est_dnn(data, l2_reg, MODEL_NAME, restore,
                                                                                  n_epoches = n_epoches, K = K, M = Num_of_layers, current_best_acc = current_best_acc)
...
# 2. return grid info
restore = True
_,_,cm_dnn_grid_output_dic = util.est_dnn(data_grid_dnn, l2_reg, MODEL_NAME, restore, n_epoches = n_epoches, K = K, M = Num_of_layers)
...
data['training'] = [cm_data['training'][x0_vars].values, cm_data['training'][x1_vars].values,
                    cm_data['training'][x2_vars].values, cm_data['training'][x3_vars].values, cm_data['training'][x4_vars].values,
                    cm_data['training'][z_vars].values, cm_data['training'][y_vars].values[:,0]] # the order: x0, x1, x2, x3, x4, z, y
data['testing'] = [cm_data['testing'][x0_vars].values, cm_data['testing'][x1_vars].values,
                   cm_data['testing'][x2_vars].values, cm_data['testing'][x3_vars].values, cm_data['testing'][x4_vars].values,
                   cm_data['testing'][z_vars].values, cm_data['testing'][y_vars].values[:,0]] # the order: x0, x1, x2, x3, x4, z, y
# 
for penalty_const in penalty_const_list:
    ...
    for replica in range(num_replications):
        _cm_resnet_params_dic,_cm_resnet_hyper_params_dic,_cm_resnet_output_dic = \
            util.est_cm_resnet_use_delta(data, cm_param_dic, penalty_const, MODEL_NAME, restore = restore, n_epoches = n_epoches, K = K,
                               current_best_acc = current_best_acc,M = Num_of_layers)
    ...
    # 2. restore 
    restore = True
    _,_,cm_resnet_grid_output_dic = util.est_cm_resnet_use_delta(data_grid, cm_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches,
                                                                 K = K,M = Num_of_layers)
...
#
with open('cm_info_use_delta_num_layer_5.pickle', 'wb') as f:
