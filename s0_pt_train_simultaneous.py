...
#    penalty_const = 0.01
    ...
    for replica in range(num_replications):
        _pt_resnet_params_dic, _pt_resnet_hyper_params_dic, _pt_resnet_output_dic = util.est_pt_resnet_simultaneous(data,
                                                                                                                 penalty_const,
                                                                                                                 MODEL_NAME,
                                                                                                                 restore,
                                                                                                                 n_epoches,
    ...
    # 2. restore
    restore = True
    _,_,pt_resnet_grid_output_dic = util.est_pt_resnet_simultaneous(data_grid, penalty_const, MODEL_NAME, restore, n_epoches)
    ...
    for epsilon in epsilon_list:
        ...
        restore = True
        _,_,pt_resnet_adv_fgsm_output=util.est_pt_resnet_simultaneous(data_adv_fgsm, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
    ...
    for epsilon in epsilon_list:
        print('tgsm ', epsilon)
        gradient_target_training,gradient_target_testing = util_helpers.tgsm_target_gradients(data, n_class, MODEL_NAME, fun = util.est_pt_resnet_simultaneous, penalty_const = penalty_const)
        data_len_list = [len(x0_var_names), len(x1_var_names), len(p0_var_names), len(p1_var_names), len(z_var_names)]
        restore = True
        data_adv_tgsm =  util_helpers.tgsm_adv_examples(data, data_len_list, epsilon, gradient_target_training, gradient_target_testing)
        _,_,pt_resnet_adv_tgsm_output=util.est_pt_resnet_simultaneous(data_adv_tgsm, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
...
#
with open('pt_info_simul.pickle', 'wb') as f:
