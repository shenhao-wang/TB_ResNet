...
for penalty_const in penalty_const_list:
    ...
    for replica in range(num_replications):
        _cm_resnet_params_dic,_cm_resnet_hyper_params_dic,_cm_resnet_output_dic = \
            util.est_cm_resnet_use_delta_save_train(data, cm_param_dic, penalty_const, MODEL_NAME, restore = restore, n_epoches = n_epoches, K = K,
    ...
    # 2. restore
    restore = True
    _,_,cm_resnet_grid_output_dic = util.est_cm_resnet_use_delta_save_train(data_grid, cm_param_dic, penalty_const, MODEL_NAME, restore, n_epoches = n_epoches, K = K)
...
#
#
with open('cm_info_use_delta_save_train.pickle', 'wb') as f:
